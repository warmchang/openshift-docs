...
* In this release of the Security Profiles Operator, the Advanced Audit Logging Framework is available as a General Availability (GA) feature. The Advanced Audit Logging Framework uses the Audit JSON Log Enricher to capture and log terminal-based command activity in {op-system-first} containers, including `oc rsh`, `oc exec`, and `oc debug` commands.
// Commented out until document goes live
// For more details, see link:https://docs.redhat.com/security/security_profiles_operator/spo-logging[Advanced Audit Logging Framework].
